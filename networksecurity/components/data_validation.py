...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            #read the data fom train and test
            ...
            if not status:
                raise NetworkSecurityException(f"Train dataframe does not have required number of columns. Required: {len(self._schema_config['columns'])}, Found: {len(train_dataframe.columns)}", sys)

            status=self.validate_number_of_columns(test_dataframe)
            if not status:
                raise NetworkSecurityException(f"Test dataframe does not have required number of columns. Required: {len(self._schema_config['columns'])}, Found: {len(test_dataframe.columns)}", sys)
